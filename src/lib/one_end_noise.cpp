...
// s512 dilution: distance is 8 [lattice unit]
// sprs1: we only use 8 single vectors from 512 diluted vectors.
...
// from this dilution, we can choose whether using all of diluted vectors or randomly taking a part of them.
// input Ndil is the number of diluted vectors we use.
// Ndil = 1 ~ 32. 
int one_end::space32_dil(std::vector<Field_F>& sdil_noise, const std::vector<Field_F>& noise_vec, const int Ndil)
{
  int Nc = CommonParameters::Nc();
  int Nd = CommonParameters::Nd();
  int Nx = CommonParameters::Nx();
  ...
  std::vector<int> index_dil(Ndil); 

  if(Ndil == 32){
    vout.general("All of s16 dilted vectors will be used. \n");
  }
  else{
    vout.general("%d s16 diluted vectors are randomly choosen.\n",Ndil);

    // randomly choose the dilution vectors
    RandomNumberManager::initialize("Mseries", time(NULL));
    RandomNumbers *rand = RandomNumberManager::getInstance();

    for(int i=0;i<Ndil;i++){
      double rnum = floor( 32.0 * rand->get() );
      index_dil[i] = (int)rnum;
    }
  }
  
  // check size 
  if(sdil_noise.size() != Nnoise*Ndil){
    vout.general("Error: size of array mismatch. \n");
    std::exit(EXIT_FAILURE);
  }
