...
int main_core(Parameters *params_conf_all)
{
  ...
  */
  ...
  // s2 dil
  one_end::space2_dil(dil_noise1,tcddil_noise1);
  std::vector<Field_F>().swap(tcddil_noise1);
  
  one_end::space2_dil(dil_noise2,tcddil_noise2);
  ...
  */
  ...
  // swapping noise1 and noise2 for bug check
...
#pragma omp parallel for
  ...
  for(int beta_src=0;beta_src<2;++beta_src){
    for(int alpha_src=0;alpha_src<2;++alpha_src){
	  ...
	  // output
          string output_4pt_base("/NBS_NN_sink%d%dsrc%d%d_swap_");
          char output_4pt[256];
