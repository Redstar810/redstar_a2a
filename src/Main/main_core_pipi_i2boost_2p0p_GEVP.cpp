...
int main_core(Parameters *params_conf_all)
{
  ...
  */
  ...
  Field_F *xi_l_local = new Field_F[Nnoise*Ndil_red];
  invtimer.start();
  a2a::inversion_alt_Clover_eo(xi_l_local, dil_noise, U, kappa_l, csw, bc,
			       Nnoise*Ndil_red, inv_prec_full,
                               Nmaxiter, Nmaxres);
  invtimer.stop();

  Field_F *xi_l = new Field_F[Nnoise*Ndil_red];
  smear_src->smear(xi_l, xi_l_local, Nnoise*Ndil_red);
  delete[] xi_l_local;

  // for nonzero total momentum (boosted frame)
  int total_mom[3] = {0,0,2};
  //int mom_solver[3] = {-total_mom[0]/2,-total_mom[1]/2,-total_mom[2]/2}; // for inversion with source momentum
  int mom_solver[3] = {-total_mom[0],-total_mom[1],-total_mom[2]}; // for inversion with source momentum
  Field_F *xi_l_mom_local = new Field_F[Nnoise*Ndil_red];
  a2a::inversion_mom_alt_Clover_eo(xi_l_mom_local, dil_noise, U, kappa_l, csw, bc, mom_solver,
                                   Nnoise*Ndil_red, inv_prec_full,
                                   Nmaxiter, Nmaxres);

  Field_F *xi_l_mom = new Field_F[Nnoise*Ndil_red];
  smear_src->smear(xi_l_mom, xi_l_mom_local, Nnoise*Ndil_red);
  delete[] xi_l_mom_local;
  ...
  */
  ...
  // calc. local sum
  //dcomplex *corr_local_pi_0mom = new dcomplex[Nt*Nsrc_t];
  dcomplex *corr_local_pi_2mom = new dcomplex[Nt*Nsrc_t];

#pragma omp parallel for
  for(int n=0;n<Nt*Nsrc_t;n++){
    //corr_local_pi_0mom[n] = cmplx(0.0,0.0);
    corr_local_pi_2mom[n] = cmplx(0.0,0.0);
  }
  int grid_coords[4];
  ...
  for(int r=0;r<Nnoise;r++){
	...
	for(int i=0;i<Ndil_tslice;i++){
	  for(int vs=0;vs<Nxyz;vs++){
	    ...
	    double pdotx = 2 * M_PI / Lx * (total_mom[0] * true_x) + 2 * M_PI / Ly * (total_mom[1] * true_y) + 2 * M_PI / Lz * (total_mom[2] * true_z);
	    for(int d=0;d<Nd;d++){
	      for(int c=0;c<Nc;c++){
		/*
		corr_local_pi_0mom[t+Nt*t_src] += cmplx(
							xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_r(c,d,vs+Nxyz*t,0) * xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_r(c,d,vs+Nxyz*t,0) + xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_i(c,d,vs+Nxyz*t,0) * xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_i(c,d,vs+Nxyz*t,0),
						    xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_i(c,d,vs+Nxyz*t,0) * xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_r(c,d,vs+Nxyz*t,0) - xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_r(c,d,vs+Nxyz*t,0) * xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_i(c,d,vs+Nxyz*t,0)
							);
		*/
		corr_local_pi_2mom[t+Nt*t_src] += cmplx(std::cos(pdotx),std::sin(pdotx)) *
		  cmplx(
			xi_l_mom[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_r(c,d,vs+Nxyz*t,0) * xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_r(c,d,vs+Nxyz*t,0) + xi_l_mom[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_i(c,d,vs+Nxyz*t,0) * xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_i(c,d,vs+Nxyz*t,0),
			xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_i(c,d,vs+Nxyz*t,0) * xi_l_mom[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_r(c,d,vs+Nxyz*t,0) - xi_l[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_r(c,d,vs+Nxyz*t,0) * xi_l_mom[i+Ndil_tslice*(t_src+Nsrc_t*r)].cmp_i(c,d,vs+Nxyz*t,0)
			);
...
#pragma omp parallel for
  for(int n=0;n<Nt*Nsrc_t;n++){
    corr_local_pi_2mom[n] /= (double)Nnoise;
    //corr_local_pi_1mom[n] /= (double)Nnoise;
  }

  //output 2pt correlator test
  string output_2pt_pi("/2pt_pi_2mom_GEVP_");
  a2a::output_2ptcorr(corr_local_pi_2mom, Nsrc_t, &timeslice_list[0], outdir_name+output_2pt_pi+timeave);

  //output_2pt_pi = "/2pt_pi_1mom_";
  //a2a::output_2ptcorr(corr_local_pi_1mom, Nsrc_t, &timeslice_list[0], outdir_name+output_2pt_pi+timeave);


  delete[] corr_local_pi_2mom;  
  //delete[] corr_local_pi_1mom;  
  delete dirac;
...
#pragma omp parallel
    ...
    idx_noise[1] = 1;
    // smeared sink
    a2a::contraction_separated_boost(Fsep_tmp, xi_l, xi_l_mom, xi_l, xi_l, idx_noise, Ndil_tslice, Nsrc_t,
				     total_mom, 0);

#pragma omp parallel 
    {
      axpy(*Fsep,1.0/2.0,*Fsep_tmp);
    }
...
#pragma omp parallel for
    ...
    string output_4pt_sep("/NBS_sep_dt0_GEVP_");
...
#pragma omp parallel for
    ...
    string output_4pt_sep("/NBS_sep_dt2_GEVP_");
    a2a::output_NBS_srctave(Fsep_o, Nsrc_t, &timeslice_list[0], outdir_name+output_4pt_sep+timeave);
...
#pragma omp parallel for
    ...
    string output_4pt_sep("/NBS_sep_dtm2_GEVP_");
    a2a::output_NBS_srctave(Fsep_o, Nsrc_t, &timeslice_list[0], outdir_name+output_4pt_sep+timeave);
...
#pragma omp parallel
    ...
    idx_noise[1] = 1;
    // smeared sink
    a2a::contraction_separated_boost(Fsep, xi_l, xi_l_mom, xi_l, xi_l, idx_noise, Ndil_tslice, Nsrc_t,
				     total_mom, 2);
...
#pragma omp parallel for
    ...
    string output_4pt_sep("/NBS_sep_dt2_");
    a2a::output_NBS_srctave(Fsep_o, Nsrc_t, &timeslice_list[0], outdir_name+output_4pt_sep+timeave);
...
#pragma omp parallel for
    ...
    string output_4pt_sep("/NBS_sep_dtm2_");
    a2a::output_NBS_srctave(Fsep_o, Nsrc_t, &timeslice_list[0], outdir_name+output_4pt_sep+timeave);
...
#pragma omp parallel
    ...
    idx_noise[1] = 1;
    // smeared sink
    a2a::contraction_connected_boost(Fconn_tmp, xi_l, xi_l_mom, xi_l, xi_l, idx_noise, Ndil_tslice, Nsrc_t,
				     total_mom, 0);

#pragma omp parallel 
    {
      axpy(*Fconn,1.0/2.0,*Fconn_tmp);
    }
...
#pragma omp parallel for
    ...
    string output_4pt_conn("/NBS_conn_dt0_GEVP_");  
...
#pragma omp parallel for
    ...
    string output_4pt_conn("/NBS_conn_dt2_GEVP_");  
    a2a::output_NBS_srctave(Fconn_o, Nsrc_t, &timeslice_list[0], outdir_name+output_4pt_conn+timeave);
...
#pragma omp parallel for
    ...
    string output_4pt_conn("/NBS_conn_dtm2_GEVP_");  
    a2a::output_NBS_srctave(Fconn_o, Nsrc_t, &timeslice_list[0], outdir_name+output_4pt_conn+timeave);
...
#pragma omp parallel
    ...
    idx_noise[1] = 1;
    // smeared sink
    a2a::contraction_connected_boost(Fconn, xi_l, xi_l_mom, xi_l, xi_l, idx_noise, Ndil_tslice, Nsrc_t,
				     total_mom, 2);
