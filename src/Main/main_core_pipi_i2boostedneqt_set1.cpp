...
int main_core(Parameters *params_conf_all)
{
  ...
  for(int r=0;r<Nnoise;r++){
    for(int t_src=0;t_src<Lt;t_src++){
      for(int t=0;t<Nt;t++){
	for(int i=0;i<Ndil_red_tslice;i++){
	  for(int vs=0;vs<Nxyz;vs++){
		...
		corr_local[t+Nt*t_src] += xi_mom[i+Ndil_red_tslice*(t_src+Lt*r)].cmp_ri(c,d,vs+Nxyz*t,0) * conj(xi[i+Ndil_red_tslice*(t_src+Lt*r)].cmp_ri(c,d,vs+Nxyz*t,0));
	      }
	    }
	  }
	}
