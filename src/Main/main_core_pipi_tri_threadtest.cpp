...
int main_core(Parameters *params_conf_all)
{
  ...
  vout.general("\nInversion (full precision)\n");
  vout.general("  prec.    = %12.6e\n", inv_prec_full);
  ...
  vout.general("\nCAA\n");
  vout.general("  transl. grid : %s\n", Parameters::to_string(caa_grid).c_str());
  vout.general("  prec.        : %12.6e\n", inv_prec_caa);
  vout.general("  seed (srcpt) : %d\n", caa_seed);
...
#pragma omp parallel
  ...
  */
  ...
  // alternative code implementation (Kanamori-san)
...
#pragma omp parallel for
  for(int srct=0;srct<Nsrc_t;srct++){
    for(int v=0;v<Nvol;v++){
      ...
      //Fbox1_p2a[v+Nvol*srct] = cmplx(Fp2aexa2[srct].cmp(0,v,0),Fp2aexa2[srct].cmp(1,v,0));
      //Fbox1_p2a[v+Nvol*srct] = -cmplx(Fp2aexa1[srct].cmp(0,v,0),Fp2aexa1[srct].cmp(1,v,0));
    }
  }
  ...
  delete[] Fbox1_p2a;
  dcomplex c1, c2, dot1, dot2;
  c1 = cmplx(1.0, 2.0);
  c2 = cmplx(2.0, 1.0);
  dot1 = conj(c1) * c2;
  dot2 = c2 * conj(c1);
  vout.general("conj(c1) * c2 = (%f,%f)\n",real(conj(c1)*c2),imag(conj(c1)*c2));
  vout.general("c2 * conj(c1) = (%f,%f)\n",real(c2*conj(c1)),imag(c2*conj(c1)));
  
  for(int t=0;t<Lt;t++){
    vout.general("%d | %16.8e | %16.8e \n",t,real(Fbox1_p2a[0+Lx*(2+Ly*3)+Lxyz*(t)]),imag(Fbox1_p2a[0+Lx*(2+Ly*3)+Lxyz*(t)]));
  }
