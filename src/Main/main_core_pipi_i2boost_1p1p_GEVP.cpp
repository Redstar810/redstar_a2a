...
int main_core(Parameters *params_conf_all)
{
  ...
  */
  ...
  //////////////////////////////////////////////////////
  // ###  make one-end vectors (w/ sink smearing) ###
  ...
  */
  ...
  dcomplex *corr_local_pi_1mom = new dcomplex[Nt*Nsrc_t];

#pragma omp parallel for
  for(int n=0;n<Nt*Nsrc_t;n++){
    corr_local_pi_0mom[n] = cmplx(0.0,0.0);
    corr_local_pi_1mom[n] = cmplx(0.0,0.0);
  }
  int grid_coords[4];
  ...
  for(int r=0;r<Nnoise;r++){
	...
	for(int i=0;i<Ndil_tslice;i++){
	  for(int vs=0;vs<Nxyz;vs++){
	    ...
	    double pdotx = 2 * M_PI / Lx * (total_mom[0]/2 * true_x) + 2 * M_PI / Ly * (total_mom[1]/2 * true_y) + 2 * M_PI / Lz * (total_mom[2]/2 * true_z);
	    for(int d=0;d<Nd;d++){
	      for(int c=0;c<Nc;c++){
		...
		corr_local_pi_1mom[t+Nt*t_src] += cmplx(std::cos(pdotx),std::sin(pdotx)) *
		  cmplx(
...
#pragma omp parallel for
  for(int n=0;n<Nt*Nsrc_t;n++){
    corr_local_pi_0mom[n] /= (double)Nnoise;
    corr_local_pi_1mom[n] /= (double)Nnoise;
  }

  //output 2pt correlator test
  string output_2pt_pi("/2pt_pi_0mom_GEVP_");
  a2a::output_2ptcorr(corr_local_pi_0mom, Nsrc_t, &timeslice_list[0], outdir_name+output_2pt_pi+timeave);

  output_2pt_pi = "/2pt_pi_1mom_GEVP_";
  a2a::output_2ptcorr(corr_local_pi_1mom, Nsrc_t, &timeslice_list[0], outdir_name+output_2pt_pi+timeave);


  delete[] corr_local_pi_0mom;  
...
#pragma omp parallel
    ...
    idx_noise[1] = 1;
    // smeared sink
    a2a::contraction_separated_boost(Fsep, xi_l, xi_l_mom, xi_l, xi_l_mom, idx_noise, Ndil_tslice, Nsrc_t,
				     total_mom, 0);
...
#pragma omp parallel
    ...
    idx_noise[1] = 1;
    // smeared sink
    a2a::contraction_connected_boost(Fconn, xi_l, xi_l_mom, xi_l, xi_l_mom, idx_noise, Ndil_tslice, Nsrc_t,
				     total_mom, 0);
