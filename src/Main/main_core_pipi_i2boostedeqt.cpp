...
int main_core(Parameters *params_conf_all)
{
	  ...
	  for(int x=0;x<Nx;x++){
		...
		for(int d=0;d<Nd;d++){
		  for(int c=0;c<Nc;c++){
		    ...
		    corr_local[t+Nt*t_src] += xi[i+Nc*Nd*2*(t_src+Nsrc_t*r)].cmp_ri(c,d,vs+Nxyz*t,0) * conj(xi[i+Nc*Nd*2*(t_src+Nsrc_t*r)].cmp_ri(c,d,vs+Nxyz*t,0));
  ...
  */
  ...
  for(int num_shift=0;num_shift<2;num_shift++){ // non-unit total momentum. 
    Field shift_tmp;
